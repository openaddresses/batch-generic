...
export default class Generic {
    ...
    constructor(pool: Pool, schema: z.ZodTypeAny) {
        if (!pool || !pool.query) throw new Err(500, new Error('Postgres Connection required'));
...
//      query.page = Params.integer(query.order, { default: 0 });
//      query.order = Params.order(query.order);
...
//   */
//  static async generate(pool: Pool, base) {
...
//          return this.deserialize(pool: Pool, pgres);
//      } catch (err) {
//          if (err.originalError && err.originalError.code && err.originalError.code === '23505') {
//              throw new Err(400, null, `${this._table} already exists`);
//          }
...
//   * @returns {Generic}
//   */
//  static async from(pool: Pool, id, opts = {}) {
//      if (!this._table && !this._view) throw new Err(500, null, 'Internal: Table or View not defined');
//      if (!opts.column) opts.column = Utils.primaryKey(this, pool);
